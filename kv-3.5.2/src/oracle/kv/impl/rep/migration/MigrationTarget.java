   ...
   */
    ...
    private synchronized void cleanup(boolean wait) {
        ...
        if (partitionDb != null) {
             ...
             * cleaning is complete. (See the isBusy() checks in the manager)
             */
    ...
    private void setRetryWait(boolean busy, Exception ex) {
        ...
        /*
         * If a non-busy error and the EOD was received then return 0 to force
         * an immediate restart
         */
        retryWait = busy ? waitAfterBusy : eodReceived ? 0 : waitAfterError;
    ...
    private synchronized DataInputStream openChannel()
        ...
        final String sourceRNName = rnNameAndPort.getFullName();

        /* getHAHostPort() returns null for a R1 node */
        if (haHostPort == null) {
            throw new IllegalStateException("Source node " + sourceRNName +
