...
public class MigrationManager implements Localizer {
     ...
     * As long as waiter.isStopped() is false, this method waits if migration
     * sources are in progress or there are completed sources waiting for ToO.
     * If, while waiting, waiter == null or waiter.isStopped() is true, false
     * is returned.
     *
     ...
     * unit tests)
     * @return true if idle, or false if waiter is null, or waiter.isStopped()
     * is true
    ...
    public boolean awaitIdle(MaintenanceThread waiter)
        ...
        while ((migrationService == null) ||
            ...
            if ((waiter == null) || waiter.isStopped()) {
                return false;
            }
    ...
    private PartitionMigrationState submitTarget(TargetRecord record) {
        ...
        /* Start only if PENDING and there is no table activity */
        if (!record.isPending() ||
            repNode.getTableManager().isBusyMaintenance()) {
    ...
    private class TargetExecutor extends ScheduledThreadPoolExecutor {
        ...
        protected void afterExecute(Runnable r, Throwable t) {
            ...
            /*
             * First check to see if we can run. If there are table ops, exit.
             * Even though this method is not synchronized, the call to
             * isBusyMaintenance() is safe  because this thread is still active
             * and awaitIdle() will not report idle before it has exited.
             *
             * Periodic calls by the admin to getStatus() will restart any
             * targets that can't be restarted here.
             */
            if (repNode.getTableManager().isBusyMaintenance()) {
                logger.log(Level.FINE,
                          "Unable to restart {0}, table operations in progress",
                          target);
