...
public class PartitionManager {
    ...
    synchronized void updateDbHandles(Topology topology,
        ...
        /*
         * If there was a active update thread, and it didn't reuse handles
         * force reuseExistingHandles to false for the new thread.
         */
        final boolean reuse = stopUpdate() ? reuseExistingHandles : false;

        updateThread = new UpdateThread(topology, repEnv, reuse);
        updateThread.start();
    ...
    /**
     * Stops the updater and waits for its thread to exit. Returns true if
     * no update thread was stopped, otherwise returns the value of
     * reuseExistingHandles of the stopped update thread.
     *
     * @return false if a thread was stopped and its reuseExistingHandles
     * was false
     */
    private boolean stopUpdate() {
        ...
        if (updateThread != null) {
            ...
            ret = updateThread.reuseExistingHandles;
            updateThread = null;
        }
        return ret;
    }
