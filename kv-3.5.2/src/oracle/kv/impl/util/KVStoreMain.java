...
public class KVStoreMain {
    ...
    private static class MakeConfigParser extends CommandParser {
        ...
        boolean isForceMakeConfig = false;
        ...
        public void verifyArgs() {
            ...
            /* Add system info */
            if (getNumCPUs() != 0) {
                parameters.setNumCPUs(getNumCPUs());
            }
            if (getMemoryMB() != 0) {
                parameters.setMemoryMB(getMemoryMB());
            }

            parameters.setForceBootstrapAdmin(isRunAdmin);

            /* Add Manager host and ports*/
            if (mgmtParser.getTrapHost() != null) {
                parameters.setMgmtTrapHost(mgmtParser.getTrapHost());
            }
            if (mgmtParser.getTrapPort() != 0) {
                parameters.setMgmtTrapPort(mgmtParser.getTrapPort());
            }
            if (mgmtParser.getPollPort() != 0) {
                parameters.setMgmtPollingPort(mgmtParser.getPollPort());
            }
