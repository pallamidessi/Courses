...
public class RegistryUtils {
    ...
    /**
     * Internal constants defining timeouts for use by utilities. These
     * constants are used solely by the method below and are a stopgap until
     * a more comprehensive solution is put in place for configuration of
     * KV utilities.
     */
    private static String DEFAULT_REGISTRY_OPEN_TIMEOUT =
        Integer.toString(KVStoreConfig.DEFAULT_REGISTRY_OPEN_TIMEOUT);

    /**
     * System property in case the above needs to be overridden.
     */
    private static String REGISTRY_OPEN_TIMEOUT_KEY =
        "registryOpenTimeout";
    /**
    ...
    public static void initRegistryCSF() {
        ...
        final String timeoutProp =
            System.getProperty(REGISTRY_OPEN_TIMEOUT_KEY,
                               DEFAULT_REGISTRY_OPEN_TIMEOUT);

        final int connectTimeOutMs = Integer.parseInt(timeoutProp);

        final SocketFactoryArgs args =
            new SocketFactoryArgs().setCsfName(registryCsfName).
            setCsfConnectTimeout(connectTimeOutMs);
    ...
    public static synchronized Registry getRegistry(
        ...
        return new ExceptionWrappingRegistry(hostname, port, csf);
    }
