...
public class DdlFuture implements oracle.kv.ExecutionFuture {
    ...
    public DdlFuture(byte[] futureBytes,
        ...
        if (statementLen > 0) {
            this.statement = readStringAndReposition(buf, statementLen);
        } else {
    ...
    private String readStringAndReposition(ByteBuffer buf, int stringLen) {
        String result = null;
        if (stringLen > 0) {
            result = new String(buf.array(), buf.position(), stringLen);
            buf.position(buf.position() + stringLen);
        }
        return result;
    }

    public DdlFuture(String statement, ExecutionInfo executionInfo,
                     DdlStatementExecutor statementExecutor, Logger logger) {
    ...
    synchronized public boolean cancel(boolean mayInterruptIfRunning)
        ...
        try {
            ...
            throw new FaultException("Problem cancelling "  +  statement +
                                     ", planId = " + planId + ": " + e, false);
    ...
    public StatementResult get(long timeout, TimeUnit unit)
        ...
        if (!finished) {
            throw new TimeoutException(statement  + " did not finish within " +
                                       timeout + " " + unit);
        }
    ...
    synchronized void applyNewInfo(ExecutionInfo newInfo, Throwable t) {
        ...
        if (newInfo.isTerminated() ||
            (newInfo.getErrorMessage() != null ||
             t != null)) {
            countdown.countDown();
        }
    }
    ...
    public static byte[] toByteArray(int planId) {
        return toByteArray(planId, new String(), null, null, null);
    }
    ...
    private static byte[] toByteArray(int planId,
        ...
        serializedLen += statementBytes.length;
        ...
        if (planId == 0) {
            if (status != null) {
                statusBytes = status.getBytes();
                statusLen = statusBytes.length;
            }

            if (statusAsJson != null) {
                statusAsJsonBytes = statusAsJson.getBytes();
        ...
        buf.putInt(planId);
        buf.putShort((short) statementBytes.length);
