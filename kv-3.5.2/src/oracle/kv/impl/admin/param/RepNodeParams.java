...
public class RepNodeParams implements ParamsWithMap, Serializable {
    ...
    /**
     * Returns the threshold number of requests used to activate statistics
     * gathering for a partition or index at an RN. That is, statistics
     * gathering is initiated if the sampled number of requests is below this
     * threshold over the period defined by {@link #getStatsLowActivePeriod}
     *
     * @return the threshold number of requests used to initiate statistics
     * gathering
     */
    public int getStatsRequestThreshold() {
        return map.getOrDefault(ParameterState.RN_SG_REQUEST_THRESHOLD).asInt();
    }
    ...
    /**
     * Gets the period (in milliseconds) for which activity must be below the
     * threshold defined by {@link #getStatsRequestThreshold()} before
     * statistics gathering activity is initiated.
     *
     * @return the period (in milliseconds) of required low request activity
     */
    public long getStatsLowActivePeriod() {
        return ParameterUtils.getDurationMillis(map,
                                        ParameterState.RN_SG_LOW_ACTIVE_PERIOD);
    }
