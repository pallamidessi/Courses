...
public class IndexRange implements FastExternalizable {
    ...
    IndexRange(IndexKeyImpl indexKey,
        ...
        if (startTarget.size() != 0) {
            ...
            /*
             * Count the number of values in the start target and key.  If they
             * are the same then this is a simple prefix match.  Use numValues()
             * rather than size() because of nested types. numValues() recurses
             * into complex types.
             */
            if (startTarget.numValues() == indexKey.numValues()) {
