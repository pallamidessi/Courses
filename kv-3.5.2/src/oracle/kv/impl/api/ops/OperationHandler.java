...
public class OperationHandler {
    ...
    int multiDelete(Transaction txn,
        ...
        final boolean moreElements = scan
            (txn, partitionId, parentKey, true /*majorPathComplete*/, subRange,
             ...
             CURSOR_DEFAULT, LockMode.RMW, Direction.FORWARD,
             new ScanVisitor() {
    ...
    private boolean reverseScan(Transaction txn,
        ...
        try {
            ...
            if (exclusiveInitKey != null) {
                ...
                status = DbInternal.search(
                    cursor, keyEntry, null, dataEntry, Search.LT, lockMode);
            } else {
     ...
     */
    private byte[] getPrefixSuccessor(byte[] prefix) {
    ...
    public List<Integer> putIfAbsentBatch(Transaction txn,
                                          PartitionId partitionId,
                                          List<KVPair> kvPairs,
                                          KVAuthorizer kvAuth) {
        ...
        try {
            for (KVPair e : kvPairs) {
                i++;
                putDatabaseEntry(keyEntry, e.getKey());
                putDatabaseEntry(dataEntry, e.getValue());
                ...
                while (true) {
                    ...
                    status = cursor.getSearchKey(keyEntry, NO_DATA,
                                                 LockMode.DEFAULT);
                    if (status == OperationStatus.SUCCESS) {
                        /* Key already exists. */
                        keysPresent.add(i);
    ...
    /**
     * As above, but reuse the supplied DatabaseEntry
     */
    private DatabaseEntry putDatabaseEntry(DatabaseEntry dbEntry,
                                           byte[] value) {
