...
bmpfile_t *bmp_create_from_file(const char *filename){
  ...
  bmpfile_t* image=bmp_create(length,height,depth);

  fseek(file,0,SEEK_SET);
  bmp_read_header(image,file);
  bmp_read_dib(image,file);

  fseek(file,image->header.offset,SEEK_SET);
  
  int current_grey;
  rgb_pixel_t pixel_grey;
  pixel_grey.alpha=0;

  for (i = 0; i < height; i++) {
    for (j = 0; j < lenght; j++) {
      fread(&current_grey,image->dib.depth,1,file);
      pixel_grey.blue=current_grey;
      pixel_grey.green=current_grey;
      pixel_grey.red=current_grey;
      image->pixel[i][j]=pixel_grey;
    }
  }
}
*/
...
static void
bmp_read_header(bmpfile_t *bmp, FILE *fp)
{
...
static void
bmp_read_dib(bmpfile_t *bmp, FILE *fp)
{
  bmp_dib_v3_header_t dib = bmp->dib;


  fread(&(dib.header_sz), sizeof(uint32_t), 1, fp);
  fread(&(dib.width), sizeof(uint32_t), 1, fp);
  fread(&(dib.height), sizeof(uint32_t), 1, fp);
  fread(&(dib.nplanes), sizeof(uint16_t), 1, fp);
  fread(&(dib.depth), sizeof(uint16_t), 1, fp);
  fread(&(dib.compress_type), sizeof(uint32_t), 1, fp);
  fread(&(dib.bmp_bytesz), sizeof(uint32_t), 1, fp);
  fread(&(dib.hres), sizeof(uint32_t), 1, fp);
  fread(&(dib.vres), sizeof(uint32_t), 1, fp);
  fread(&(dib.ncolors), sizeof(uint32_t), 1, fp);
